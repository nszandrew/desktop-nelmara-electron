...
export default function CreateTreatmentTemplate() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.post("/treatment-templates", payload, {
        headers: { Authorization: `Bearer ${token}` },
      });
      alert("Template criado com sucesso!");
      navigate("/treatments"); // <- ajuste aqui
    } catch (err) {
      ...
      >
        <div style={styles.card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <h2 style={styles.title}>📝 Criar Template de Tratamento</h2>
            <button onClick={() => navigate("/templates")} style={styles.backBtn}>
              <FaArrowLeft /> Voltar
            </button>
          </div>
